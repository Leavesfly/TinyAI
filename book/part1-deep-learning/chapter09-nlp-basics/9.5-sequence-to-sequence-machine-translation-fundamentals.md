...
> **本节学习目标**：掌握序列到序列模型的基本原理和实现，理解注意力机制原理，实现简单的机器翻译任务

## 内容概览

序列到序列（Sequence-to-Sequence, Seq2Seq）模型是处理序列到序列映射任务的重要架构，在机器翻译、文本摘要、对话系统等领域有着广泛应用。本节将深入学习Seq2Seq模型的核心原理，实现编码器-解码器架构，并介绍注意力机制这一革命性技术。

## 9.5.1 序列到序列模型概述

序列到序列模型旨在解决输入序列和输出序列长度不同的问题。与传统的固定输入输出模型不同，Seq2Seq模型能够处理变长的输入和输出序列。
...
Seq2Seq模型采用编码器-解码器（Encoder-Decoder）架构：
