package io.leavesfly.tinyai.mlearning.evaluator;
...
public class EvaluatorTest {
    ...
    private static class TestDataSet extends DataSet {
        ...
        public java.util.Map<String, DataSet> splitDataset(float trainRatio, float testRatio, float validaRation) {
            if (Math.abs(trainRatio + testRatio + validaRation - 1.0f) > 1e-6) {
                throw new RuntimeException("splitDataset parameters error!");
            }
            
            java.util.Map<String, DataSet> result = new java.util.HashMap<>();
