...
多模态检索是指能够处理和理解不同类型数据(文本、图像、音频、视频、代码等)的检索技术。其核心思想是将不同模态的数据映射到统一的向量空间,实现跨模态的语义理解和检索。
...
graph TB
    ...
    subgraph "模态编码器"
        B1[文本编码器<br/>BERT/GPT]
        B2[图像编码器<br/>ResNet/ViT]
        B3[代码编码器<br/>CodeBERT]
        B4[音频编码器<br/>Wav2Vec]
    end
    
    subgraph "统一向量空间"
        C[共享语义空间<br/>512/768维]
    end
...
| 以图搜文 | 图像 | 文本 | 图片描述生成 |
| 以文搜图 | 文本 | 图像 | 图片搜索引擎 |
| 代码搜索 | 自然语言 | 代码 | GitHub Copilot |
| 视频理解 | 视频 | 文本 | 视频摘要生成 |

## 文本向量化技术

现代文本向量化主要基于预训练语言模型,能够捕获深层语义信息。

### 主流文本编码器对比

| 模型 | 参数量 | 维度 | 优势 | 适用场景 |
|------|--------|------|------|----------|
| BERT | 110M-340M | 768 | 双向理解 | 句子相似度 |
| Sentence-BERT | 110M | 768 | 快速推理 | 语义检索 |
| GPT | 117M-175B | 768-12288 | 生成能力 | 文本生成 |
| CLIP-Text | 63M | 512 | 跨模态 | 图文匹配 |
...
public class TextEncoder {
    ...
    public TextEncoder(String modelName, int dimension) {
    ...
    public float[] encode(String text) {
        ...
        // 2. 转换为输入ID
        int[] inputIds = tokensToIds(tokens);
        
        // 3. 模型推理
        float[] embedding = model.forward(inputIds);
        
        // 4. 池化(如使用[CLS]或平均池化)
        return pooling(embedding);
    }
