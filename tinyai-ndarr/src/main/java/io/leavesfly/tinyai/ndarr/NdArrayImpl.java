...
/**
 * N维数组实现类，支持标量、向量、矩阵等多维数据结构
 *
 * <p>该类经过重构优化，提供更加优雅的API和更好的性能，是深度学习框架的核心数据结构实现。</p>
 *
 * <p>主要特性：</p>
 * <ul>
 *   <li>支持任意维度的数组操作</li>
 ...
 * @since 1.0
 */
public class NdArrayImpl implements INdArray {
...
//    private static final Random RANDOM = new Random();
     ...
     * @param number 标量值
     */
    public NdArrayImpl(Number number) {
        this.shape = new ShapeImpl(1, 1);
     ...
     */
    public NdArrayImpl(float[] data, IShape shape) {
     ...
     * @param data 一维数据数组
     */
    public NdArrayImpl(float[] data) {
        this.shape = new ShapeImpl(1, data.length);
        this.buffer = data;
    }
     ...
     */
    public NdArrayImpl(Object data) {
     ...
     * @param shape 数组形状
     */
    public NdArrayImpl(IShape shape) {
        this.shape = shape;
    ...
    private void initFromArray(float[][] data) {
        ...
        this.shape = new ShapeImpl(data.length, data[0].length);
        this.buffer = new float[shape.size()];
    ...
    private void initFromArray(float[][][] data) {
        validateArrayDimensions(data);
        this.shape = new ShapeImpl(data.length, data[0].length, data[0][0].length);
        this.buffer = new float[shape.size()];
        flattenArray(data, this.buffer, 0);
    }
    ...
    private void initFromArray(float[][][][] data) {
        validateArrayDimensions(data);
        this.shape = new ShapeImpl(data.length, data[0].length, data[0][0].length, data[0][0][0].length);
        this.buffer = new float[shape.size()];
        flattenArray(data, this.buffer, 0);
    }
     ...
     */
    private static void validateMatrixShape(IShape shape) {
        if (!shape.isMatrix()) {
     ...
     * @return 全零数组
     */
    public static INdArray zeros(IShape shape) {
        return new NdArrayImpl(shape); // 默认就是全零
    }
     ...
     * @return 全一数组
     */
    public static INdArray ones(IShape shape) {
        NdArrayImpl result = new NdArrayImpl(shape);
        result.fillAll(1.0f);
        return result;
    }
     ...
     */
    public static INdArray eye(IShape shape) {
     ...
     * @return 指定值填充的数组
     */
    public static INdArray like(IShape shape, Number value) {
        NdArrayImpl result = new NdArrayImpl(shape);
     ...
     * @return 指定值填充的数组
     */
    public INdArray like(Number value) {
        return NdArrayImpl.like(this.shape, value);
    }
     ...
     * @return 标准正态分布随机数组
     */
    public static INdArray likeRandomN(IShape shape) {
     ...
     * @return 标准正态分布随机数组
     */
    public static INdArray likeRandomN(IShape shape, long seed) {
        NdArrayImpl result = new NdArrayImpl(shape);
     ...
     * @return 均匀分布随机数组
     */
    public static INdArray likeRandom(float min, float max, IShape shape) {
     ...
     * @return 均匀分布随机数组
     */
    public static INdArray likeRandom(float min, float max, IShape shape, long seed) {
        NdArrayImpl result = new NdArrayImpl(shape);
        Random random = new Random(seed);
        for (int i = 0; i < result.buffer.length; i++) {
     ...
     */
    public static INdArray linSpace(float min, float max, int num) {
        if (num <= 0) {
            throw new IllegalArgumentException("数量必须大于0");
        }
        INdArray result = likeRandom(min, max, new ShapeImpl(1, num));
        Arrays.sort(((NdArrayImpl)result).buffer);
        return result;
    }
     ...
     */
    private INdArray binaryOperation(INdArray other, BinaryOperator<Float> operation, String operationName) {
        validateShapeCompatibility(this.shape, other.getShape(), operationName);
        NdArrayImpl result = new NdArrayImpl(this.shape);
        NdArrayImpl otherImpl = (NdArrayImpl) other;
        for (int i = 0; i < this.buffer.length; i++) {
            result.buffer[i] = operation.apply(this.buffer[i], otherImpl.buffer[i]);
        }
        return result;
    }
     ...
     * @return 运算结果数组
     */
    private INdArray scalarOperation(Number scalar, BinaryOperator<Float> operation) {
        NdArrayImpl result = new NdArrayImpl(this.shape);
     ...
     */
    private static void validateShapeCompatibility(IShape shape1, IShape shape2, String operationName) {
     ...
     * @return 乘法运算结果
     */
    public INdArray mulNum(Number number) {
     ...
     */
    public INdArray divNum(Number number) {
